...
const EditTask = ({ title, name, desc, status }: TaskProps) => {
    const [modalVisible, setModalVisible] = useState(false);
    const [isOpen, setOpen] = useState<boolean>(false);
    const [inputName, setInputName] = useState<string>(name);
    const [inputDesc, setInputDesc] = useState<string>(desc);
    ...
    const handleSubmit = () => {
        if (inputName === '' || inputDesc === '' || inputStatus === '') {
            setWarning(true);
        } else {
            setWarning(false);
    ...
    const handleSelect = (selectedStatus: string) => {
        setInputStatus(selectedStatus);
    };

    return (
        <div>
            {modalVisible && (
                <div className="center-screen min-w-min">
                    <div
                    ...
                    >
                        <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                            ...
                            <div className="flex items-center justify-start w-full">
                                <button
                                    type="submit"
                                    className="w-full text-center focus:outline-none transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 text-sm"
            ...
            <div className="w-full flex justify-center " id="button">
                <button
                    className="font-bold focus:outline-none mx-auto transition duration-150 ease-in-out rounded  text-xs sm:text-sm"
                    onClick={openModal}
                >
                    {title}
                </button>
